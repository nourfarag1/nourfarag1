...
My day-to-day involves working across a **.NET microservices architecture** — designing background job pipelines, integrating third-party services, building multi-tenant data flows, and occasionally getting my hands dirty with Linux servers, Docker, and CI/CD pipelines. I'm also building **Argus**, a self-hostable monitoring tool in **Python & Django**, as a deliberate deep-dive into cloud-native infrastructure — containers, reverse proxies, message brokers, caching, and orchestration — the patterns I don't get to use day-to-day in .NET.
...
- **Backend:** REST APIs, Django REST Framework, CQRS/MediatR, Background Job Orchestration (Hangfire, Quartz.NET), Multi-tenant Architecture, Clean Architecture
- **Languages:** C# · Python
...
### 🔍 [Argus](https://github.com/nourfarag1/Argus) *(In Progress)*
A self-hostable, early-warning uptime & monitoring tool built with **Python · Django · DRF**, designed from scratch as a self-directed learning project covering the full backend infrastructure stack.

- Containerized a Django REST API, a background worker, PostgreSQL, and an Nginx reverse proxy with Docker Compose
- Isolated internal services behind Nginx as the single public entry point; added a one-shot migration service for deterministic, race-free startup
- Documented architecture decisions as ADRs and structured the build as a versioned, phased roadmap
- Next: async alerting via a message broker (Celery), distributed caching (Redis), load balancing, and Kubernetes
